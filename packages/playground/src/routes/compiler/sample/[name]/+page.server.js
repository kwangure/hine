...
import { TEST_COMPILER_SAMPLES_PATH } from '$tests';

export function load({ params }) {
	const { name } = params;
	const sampleDirNames = fs
		.readdirSync(TEST_COMPILER_SAMPLES_PATH, { withFileTypes: true })
	...
	return {
		...
		sample: {
			name,
			content: fs.readFileSync(`${TEST_COMPILER_SAMPLES_PATH}/${name}/input.hine`, 'utf-8'),
		},
	};
}
